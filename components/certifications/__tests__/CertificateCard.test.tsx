...
describe("CertificateCard", () => {
    it("renders card with certificate type", () => {
        ...
        expect(
            screen.getByText((_, node: any) =>
                node.textContent === `Credential ID: ${baseProps.credential_id}`
            )
        ).toBeInTheDocument();
        expect(screen.getByText("View Certificate →")).toHaveAttribute("href", baseProps.link);
    ...
    it("renders card with certification type and badge", () => {
        ...
        expect(screen.getByText("View Certificate →")).toHaveClass("text-warning-600");
    });
    ...
    it("renders both logos with correct alt text", () => {
        ...
        const images = screen.getAllByRole("img", { name: `${baseProps.issuer} logo` });
